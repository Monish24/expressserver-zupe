...
import { getAlbumLibSongs, getAlbumsFromLib, getArtistLibSongs, getArtistsFromLib, getSongsFromLib, saveToLibrary } from "../../database"
...
export function ulib() {
  ...
  app.post('/lib/getlibalbmsongs', async (req, res) => {
    const albm_id = req.body.albumid
    const albmsongs = await getAlbumLibSongs(uid,albm_id)
    // const result = [...new Set(albmsongs.flatMap(({song_id}) => song_id))].sort()
    console.log(albmsongs)
    res.send(albmsongs)
  })
}
