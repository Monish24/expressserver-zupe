...
export async function getSongsFromLib(userlib: userlibrary) {
  const repo = connection.getRepository(userlibrary)

  const librarysongs = await repo.query(`SELECT song_id FROM userlibrary WHERE uid = ?`, [userlib.uid]) as userlibrary[]
